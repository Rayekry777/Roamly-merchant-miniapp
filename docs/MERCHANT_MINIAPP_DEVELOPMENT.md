...
- 商户端固定使用 `lottie-miniprogram@1.0.12` 和 Canvas 2D 播放本地 Roamly 动画数据，不在运行时下载动画资源。
- Lottie 只用于核销、撤销、入驻提交、团购券提交和接受员工邀请等明确完成态；页面进入、按钮按压、筛选切换、卡片展开和数据刷新使用 WXSS 动画。
- 动画不得阻断业务结果或提供额外业务状态。低性能设备、Canvas 初始化失败或动画关闭时退化为静态成功图标和文字。
- 动画禁止无限循环；重复触发前销毁旧实例，组件隐藏、页面卸载或播放结束时清理实例和定时器。

## 登录、个人信息与账号状态

- 登录页提供验证码登录、注册并进入、密码登录三种模式；验证码按 `LOGIN`（登录）和 `REGISTRATION`（注册）隔离，短信登录不隐式创建商户。
- 注册创建 `VISITOR`（游客）、`NOT_APPLIED`（未入驻）账号；入驻审核通过后成为 `TENANT`（租户）。密码为 8–64 位且同时包含字母和数字。Token 域固定为 `MERCHANT`（商户端），消费者或管理员 Token 不得写入本端存储。
- “账号与门店”弹层固定在“进入管理版”前展示“个人信息”。个人信息页允许维护头像、昵称、手机号和密码，只读展示角色、状态与所属门店，不提供性别、生日或门店资料编辑。
- 头像先以 `MERCHANT_AVATAR`（商户头像）用途上传并绑定，展示时通过 Bearer 私有内容接口下载为本地临时路径，不缓存对象存储地址。
- 手机号与密码修改沿用消费者端输入规则；成功后清理商户 Store、券草稿与本地 Token，并重新进入登录页。退出登录仍使用“账号与门店”弹层原有确认、服务端注销和本地清理流程。
...
| 能力       | `VISITOR`（游客） | `TENANT`（租户） | `MANAGER`（店长） | `VERIFIER`（核销员） |
| ---------- | ----------------- | ---------------- | ----------------- | -------------------- |
| 门店资料   | 入驻申请          | 管理             | 查看              | 不展示               |
| 团购券     | 不展示            | 创建、提交、下架 | 创建、提交、下架  | 不展示               |
| 订单       | 不展示            | 全部门店订单     | 全部门店订单      | 不展示               |
| 核销       | 不展示            | 扫码、手输、撤销 | 扫码、手输、撤销  | 扫码、手输           |
| 核销记录   | 不展示            | 全部             | 全部              | 仅本人               |
| 员工       | 接受邀请          | 邀请、停用、查看 | 不展示            | 不展示               |
| 佣金和结算 | 不展示            | 查看             | 查看              | 不展示               |
...
- 底部导航固定为“工作台、经营、我的”。订单、核销、核销记录、团购券、员工与结算作为独立业务页面由工作台或经营页进入。
- 工作台前置扫码与手输核销，展示“本店今日收银总计”；今日数据仅消费 `/v1/merchant/finance/today` 聚合接口，不从本地列表推算。
- 经营页按交易、商品、财务和团队分组，并按 `current.permissions` 隐藏无权限入口；核销员只展示核销能力。
- 我的展示头像、账号状态、角色、门店、入驻或邀请入口、可用经营入口和退出登录。
- 详情使用稳定页面或底部抽屉；不可将多个业务页面堆成嵌套卡片。
- 今日数据进入 `/pages/today/index`，按团购核销券数、退款券数、核销金额、退款金额和净收展示；核销明细进入 `/pages/redemptions/index`，支持全部、已核销、可撤销、已撤销、已退款五个页签、订单/券码搜索和详情页收入拆解。

## 入驻

- 游客填写营业执照、联系人、类目、城市、地址、地图坐标、营业时间、经营图片和 Mock 结算资料。
...
- 租户输入已注册游客手机号后创建 60 秒有效、单次使用的六位数字凭证，并指定 `MANAGER`（店长）或 `VERIFIER`（核销员）；邀请链路不生成二维码。
- 核销支持 `wx.scanCode({onlyFromCamera:true,scanType:['qrCode']})` 和手输 12 位编号；用户取消扫码、相机拒绝、二维码过期和服务失败使用不同反馈。
- 扫码与手输统一进入预览页，展示商品、券型、券面权益、有效期、剩余次数和使用规则；不采集线下消费金额。
- 确认核销携带预览令牌与 `Idempotency-Key`；成功页不得允许重复确认。
- 租户和店长可撤销尚未结算的误核销，必须填写原因；核销员不显示撤销入口。
- 核销记录页面消费 `GET /v1/merchant/redemptions`；核销员客户端仅展示 `operatorId` 为当前账号的记录，租户和店长展示门店记录。
...
- 认证消费 `POST /v1/merchant/auth/sms-codes`、`POST /v1/merchant/auth/login`、`POST /v1/merchant/auth/registrations`、`POST /v1/merchant/auth/password-sessions`、`GET /v1/merchant/auth/me` 与 `POST /v1/merchant/auth/logout`。
- 账号资料消费 `/v1/merchant/account/profile`、`nickname`、`avatar`、`phone-change/sms-codes`、`phone`、`password-change/sms-codes` 与 `password`；当前身份只展示脱敏手机号，个人信息页才接收完整手机号。
...
- 售后页调用 `/v1/merchant/after-sales` 查看本店退款申请，租户和店长可发起退款申请；商户不执行审批、驳回或资金变更，平台统一处理并通过 `REFUND_UPDATED` 刷新结果。
- 订单入口在客户端只对具备 `merchant:order:read`（商户订单查看）权限的 `TENANT`（租户）和 `MANAGER`（店长）显示；`VERIFIER`（核销员）不展示入口，若直接访问接口则展示后端 `MERCHANT_FORBIDDEN`（商户无权操作）错误，不伪装为空列表。
