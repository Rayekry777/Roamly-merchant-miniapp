...
export function listStaff(page = 1, size = 20) {
  return request<PageResult<MerchantStaff>>(
    `/v1/merchant/staff?page=${page}&size=${size}`,
  );
}
export function inviteStaff(phone: string, role: "MANAGER" | "VERIFIER") {
  return request<StaffInvitation>("/v1/merchant/staff-invitations", {
    method: "POST",
    data: { phone, role },
    headers: { "Idempotency-Key": `staff-${Date.now()}` },
  });
}
export function disableStaff(id: string) {
  return request<MerchantStaff>(`/v1/merchant/staff/${id}/disablement`, {
    method: "POST",
    headers: { "Idempotency-Key": `staff-disable-${id}-${Date.now()}` },
  });
}
export function activateStaff(id: string) {
  return request<MerchantStaff>(`/v1/merchant/staff/${id}/activation`, {
    method: "POST",
